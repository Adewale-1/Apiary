...
def load_project_contract(path: str | Path) -> ProjectContract:
    ...
    # Support both [autoresearch] (new) and [project] (legacy) section names.
    ar = payload.get("autoresearch") or payload.get("project", {}) # TODO: SHOULD only support autoresearch since we changed project.toml to autoresearch.toml
